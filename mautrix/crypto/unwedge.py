...
from mautrix.types import UserID, IdentityKey, EventType, Obj
    ...
    async def _unwedge_session(self, sender: UserID, sender_key: IdentityKey) -> None:
        try:
            ...
            if delta < MIN_UNWEDGE_INTERVAL:
                self.log.debug(f"Not creating new Olm session with {sender}/{sender_key}, "
        ...
        try:
            ...
            if device is None:
                self.log.warning(f"Didn't find identity of {sender}/{sender_key},"
                                 " can't unwedge session")
                return
            self.log.debug(f"Creating new Olm session with {sender}/{device.user_id} "
                           f"(key: {sender_key})")
            await self.send_encrypted_to_device(device, EventType.TO_DEVICE_DUMMY, Obj(),
