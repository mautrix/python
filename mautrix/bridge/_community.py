...
class CommunityHelper:
    ...
    async def update(self, community_id: CommunityID, name: Optional[str] = None,
                     avatar_url: Optional[ContentURI] = None, short_desc: Optional[str] = None,
                     long_desc: Optional[str] = None) -> None:
        if not community_id:
            return
        await self.az.intent.api.request(Method.POST, Path.groups[community_id].profile,
                                         {k: v for k, v in {
                                             "name": name,
    ...
    async def invite(self, community_id: CommunityID, user_id: UserID) -> None:
        ...
        try:
            await self.az.intent.api.request(Method.PUT,
                                             Path.groups[community_id].admin.users.invite[user_id])
        except MatrixStandardRequestError as e:
